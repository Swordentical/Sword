...
- **Comprehensive Data Backup**: Full system backup (v2.3) includes all clinical data (patients with full profiles including medical history/treatments/chart/documents, appointments past and future, treatments/services), financial data (invoices with items and adjustments, payments, payment plans with installments, expenses, insurance claims, doctor payments), lab data (lab cases, external labs, lab services), inventory, users (without passwords), clinic settings/rooms. Restore preserves original IDs to maintain all relationships.
